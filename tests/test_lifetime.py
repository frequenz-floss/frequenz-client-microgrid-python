...
def test_creation(now: datetime, future: datetime, case: _LifetimeTestCase) -> None:
    ...
    assert lifetime.active == case.expected_active
...
def test_validation(
    ...
    Args:
        past: Past datetime fixture
        now: Current datetime fixture
        future: Future datetime fixture
        start: Start time type
        end: End time type
    """
    time_map = {
        _Time.PAST: past,
...
def test_active_property(
    ...
    lifetime = Lifetime(start=start_time, end=end_time)
    assert lifetime.active_at(now) == case.expected_active


@pytest.mark.parametrize(
    "case",
    [
        _FixedLifetimeTestCase(name="past", test_time=_Time.PAST, expected_active=True),
        _FixedLifetimeTestCase(name="now", test_time=_Time.NOW, expected_active=True),
        _FixedLifetimeTestCase(
            name="future", test_time=_Time.FUTURE, expected_active=True
        ),
    ],
    ids=lambda case: case.name,
)
def test_active_at_with_fixed_lifetime(
    ...
    Tests a lifetime from past to future and checks activity at different points:
    - Past timestamp: should be active
    - Current timestamp: should be active
    - Future timestamp (equal to end): should be active (inclusive)
    ...
    assert lifetime.active_at(test_time) == case.expected_active
