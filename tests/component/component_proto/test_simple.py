...
def test_category_mismatch(default_component_base_data: ComponentBaseData) -> None:
    """Test MismatchedCategoryComponent for category GRID and battery metadata."""
    major_issues: list[str] = []
    minor_issues: list[str] = []
...
def test_voltage_transformer(
    ...
    minor_issues: list[str] = []
    base_data = default_component_base_data._replace(
        category=ComponentCategory.VOLTAGE_TRANSFORMER
    )

    proto = base_data_as_proto(base_data)
    if primary is not None:
        proto.category_type.voltage_transformer.primary = primary
    if secondary is not None:
        proto.category_type.voltage_transformer.secondary = secondary
...
def test_grid(
    ...
    major_issues: list[str] = []
    minor_issues: list[str] = []
    base_data = default_component_base_data._replace(category=ComponentCategory.GRID)

    proto = base_data_as_proto(base_data)
    if rated_fuse_current is not None:
        proto.category_type.grid.rated_fuse_current = rated_fuse_current
...
def test_issues_logging(
    ...
    mock_proto = Mock(name="proto", spec=components_pb2.Component)
